...
public class HudManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (test)
        {
            ApplyDamage(5); // Ejemplo: aplica 20 de da�o
            test = false;
        }
    }
    ...
    public void ApplyDamage(int totalDamage)
    {
        ...
        if (currentArmor > 0)
        {
            int healthDamage = Mathf.CeilToInt(totalDamage * distributionFactor); // 1/5 del da�o total a la vida
            int armorDamage = totalDamage - healthDamage;           // 4/5 del da�o total a la armadura
